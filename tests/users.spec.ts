...
test.describe('Users Module - 사용자 관리', () => {
  ...
  test('TC-USER-001: 새 사용자를 생성할 수 있다', async ({ page }) => {
    ...
    // And: 사용자 정보 입력
    await page.getByLabel('이름').fill(testName);
    await page.getByLabel('이메일').fill(testEmail);

    // And: 시간대 및 언어 설정
    await page.locator('select').filter({ hasText: 'UTC' }).selectOption('Asia/Seoul');
    await page.locator('select').filter({ hasText: 'English' }).selectOption('ko');
  ...
  test('TC-USER-003: 이름 없이 사용자 생성 시 에러 표시', async ({ page }) => {
    ...
    await page.getByLabel('이메일').fill(`empty-name-${uniqueId}@example.com`);
    await page.getByRole('button', { name: '생성' }).click();
  ...
  test('TC-USER-004: 중복된 이메일로 사용자 생성 시 에러 표시', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '사용자 생성' }).click();
    await page.getByLabel('이름').fill('첫번째사용자');
    await page.getByLabel('이메일').fill(duplicateEmail);
    await page.getByRole('button', { name: '생성' }).click();
    await expect(page.getByText('첫번째사용자')).toBeVisible();

    // When: 동일한 이메일로 다시 생성 시도
    await page.getByRole('button', { name: '사용자 생성' }).click();
    await page.getByLabel('이름').fill('중복사용자');
    await page.getByLabel('이메일').fill(duplicateEmail);
    await page.getByRole('button', { name: '생성' }).click();
  ...
  test('TC-USER-005: 사용자 정보를 수정할 수 있다', async ({ page }) => {
    ...
    // And: 시간대 변경
    await page.locator('select').filter({ hasText: /UTC|Asia/ }).first().selectOption('Asia/Tokyo');
  ...
  test('TC-USER-006: 사용자를 삭제할 수 있다', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '사용자 생성' }).click();
    await page.getByLabel('이름').fill('삭제용사용자');
    await page.getByLabel('이메일').fill(deleteEmail);
    await page.getByRole('button', { name: '생성' }).click();
    await expect(page.getByText('삭제용사용자')).toBeVisible();

    // When: 해당 사용자의 삭제 버튼 클릭
    const targetRow = page.locator('tr').filter({ hasText: '삭제용사용자' });
    ...
    await expect(page.getByText('삭제용사용자')).not.toBeVisible();
  });
